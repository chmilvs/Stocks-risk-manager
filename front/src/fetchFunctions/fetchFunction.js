...
export const fetchTickers = (tickerName, setLoading, setInfo, setBtnShow, setFailureMssg, setActualPrice) => {
    if (tickerName !== null) {
        console.log('here1')
        let URL = `https://financialmodelingprep.com/api/v3/historical-chart/15min/${tickerName}${API_KEY}`
        setLoading(true)
        fetch(URL)
            .then(res => res.json())
            .then(data => {
        ...
        setLoading(true)
        let urlForActualPrice = `https://financialmodelingprep.com/api/v3/quote-short/${tickerName}${API_KEY}`
...
export const refreshActualPrice = (tickerName, setFailureMssg, setActualPrice, setBtnShow, setLoading) => {
    ...
    let urlForActualPrice2 = `https://financialmodelingprep.com/api/v3/quote-short/${tickerName}${API_KEY}`
...
export const refreshData = (event, tickerName, setLoading, setFailureMssg, setInfo, setBtnShow) => {
    ...
    let URL2 = `https://financialmodelingprep.com/api/v3/${time}/${tickerName}${API_KEY}`
    setLoading(true)
    fetch(URL2)
        .then(res => res.json())
        .then(data => {
